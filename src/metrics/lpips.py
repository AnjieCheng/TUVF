...
def compute_lpips(dir, out=None, N=None, all_pairs=False, use_gpu=True, dump=False):
    ## Initializing the model
    ...
    dists = []
    for ff, file in tqdm(enumerate(files[:-1])):
        img0 = lpips.im2tensor(lpips.load_image(os.path.join(dir, file)))  # RGB image from [-1,1]
        if use_gpu:
            img0 = img0.cuda()
        ...
        for file1 in files1:
            img1 = lpips.im2tensor(lpips.load_image(os.path.join(dir, file1)))
