...
HYDRA_ARGS = "hydra.output_subdir=null hydra/job_logging=disabled hydra/hydra_logging=disabled hydra.run.dir=. hydra.job.chdir=True"
...
def main(cfg: DictConfig):
    ...
    cfg.env.project_path = str(cfg.env.project_path)
    CONSOLE.log("Project Path : {}".format(cfg.env.project_path))
    ...
    training_cmd = (
        f"{before_train_cmd}TORCH_EXTENSIONS_DIR={torch_extensions_dir} cd {cfg.experiment_dir} && PYTHONPATH=. {cfg.env.python_bin} src/train.py hydra.run.dir={cfg.experiment_dir} {HYDRA_ARGS}"
    )
    ...
    if not quiet:
        CONSOLE.log("<=== TRAINING COMMAND START ===>")
        CONSOLE.log(training_cmd)
        CONSOLE.log("<=== TRAINING COMMAND END ===>")
    ...
    if cfg.training.resume == "latest" and os.path.isdir(cfg.experiment_dir) and os.path.isfile(training_cmd_save_path) and os.path.isfile(cfg_save_path):
        ...
        if not quiet:
            CONSOLE.log("We are going to resume the training and the experiment already exists. " "That's why the provided config/training_cmd are discarded and the project dir is not created.")
        ...
        with open(training_cmd_save_path) as f:
            ...
            CONSOLE.log("<=== NEW TRAINING COMMAND START ===>")
            CONSOLE.log(training_cmd)
            CONSOLE.log("<=== NEW TRAINING COMMAND END ===>")
    else:
        CONSOLE.log("Running from scratch...")
    ...
    if is_running_from_scratch and not cfg.print_only:
        ...
        with open(training_cmd_save_path, "w") as f:
            ...
            if not quiet:
                CONSOLE.log(f"Saved training command in {training_cmd_save_path}")

        with open(cfg_save_path, "w") as f:
            OmegaConf.save(config=cfg, f=f)
            if not quiet:
                CONSOLE.log(f"Saved config in {cfg_save_path}")
