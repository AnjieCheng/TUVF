...
class DiscriminatorBlock(torch.nn.Module):
    def __init__(
        ...
        self.fromrgb = Conv2dLayer(
            img_channels, tmp_channels, kernel_size=1, activation=activation, c_dim=c_dim, hyper_mod=False, trainable=next(trainable_iter), conv_clamp=conv_clamp, channels_last=self.channels_last
        )
        self.conv0 = Conv2dLayer(
            tmp_channels, tmp_channels, kernel_size=3, activation=activation, c_dim=c_dim, hyper_mod=False, trainable=next(trainable_iter), conv_clamp=conv_clamp, channels_last=self.channels_last
        )
    ...
    def __init__(
        ...
        if self.hyper_mod:
            ...
            self.hyper_mod_mapping = MappingNetwork(
                z_dim=0, c_dim=self.scalar_enc.get_dim(), camera_cond=False, camera_cond_drop_p=0.0, w_dim=hyper_mod_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs
            )
        else:
        ...
        if self.c_dim > 0 or not self.scalar_enc is None:
            self.head_mapping = MappingNetwork(
                z_dim=0, c_dim=total_conditioning_dim, camera_cond=self.cfg.camera_cond, camera_cond_drop_p=self.cfg.camera_cond_drop_p, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs
            )
        else:
            self.head_mapping = None
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)

    def forward(self, img, global_img: torch.Tensor = None, c: torch.Tensor = None, patch_params: torch.Tensor = None, camera_angles: torch.Tensor = None, update_emas=False, **block_kwargs):
