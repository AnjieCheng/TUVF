...
class StyleGAN2Loss(Loss):
    def __init__(self, cfg, device, G, D, augment_pipe=None, r1_gamma=10, style_mixing_prob=0, pl_batch_shrink=2, pl_decay=0.01):
        ...
        self.blur_init_sigma = cfg.training.loss_kwargs.get("blur_init_sigma", 0)
        self.blur_fade_kimg = cfg.training.loss_kwargs.get("blur_fade_kimg", 0)
    ...
    def run_D(self, img_mask, blur_sigma=0, key="None", update_emas=False, cur_nimg=None, wandb_run=None, **kwargs):
...
class CanonicalStyleGAN2Loss(StyleGAN2Loss):
    def run_G(self, z, camera_angles, points, folding, camera_angles_cond=None, update_emas=False, verbose=False):
        patch_params = sample_patch_params(len(z), self.patch_cfg, device=z.device) if self.patch_cfg["enabled"] else {}
        patch_kwargs = dict(patch_params=patch_params) if self.patch_cfg["enabled"] else dict(patch_params=None)
        # import pdb; pdb.set_trace()

        if verbose:
            img, G_info = self.G.synthesis(z, camera_angles, points=points, f=folding, update_emas=update_emas, verbose=True, **patch_kwargs)
            return img, [ZeroDivisionError()], patch_params, G_info
        else:
            img = self.G.synthesis(z, camera_angles, points=points, f=folding, update_emas=update_emas, verbose=False, **patch_kwargs)
            return img, [z], patch_params

    def accumulate_gradients(self, phase, real_img, real_camera_angles, gen_z, gen_camera_angles, gen_camera_angles_cond, gain, cur_nimg, points, folding, wandb_run):
        with torch.autograd.set_detect_anomaly(True):
            ...
            if phase in ["Greg_pl", "Gall"] and self.cfg.model.loss_kwargs.pl_weight > 0:
                with torch.autograd.profiler.record_function("Gpl_forward"):
                    gen_img, _gen_ws, patch_params = self.run_G(gen_z, gen_camera_angles, camera_angles_cond=gen_camera_angles_cond, verbose=False, points=points, folding=folding)
                    pl_noise = torch.randn_like(gen_img) / np.sqrt(gen_img.shape[2] * gen_img.shape[3])
                    with torch.autograd.profiler.record_function("pl_grads"), conv2d_gradfix.no_weight_gradients():
            ...
            if phase in ["Gmain", "Greg_mvc", "Gall"]:
                with torch.autograd.profiler.record_function("Gmain_forward"):
                    gen_img, _gen_ws, patch_params, info = self.run_G(gen_z, gen_camera_angles, camera_angles_cond=gen_camera_angles_cond, verbose=True, points=points, folding=folding)
                    gen_logits = self.run_D(gen_img, key="gen", blur_sigma=blur_sigma, patch_params=patch_params, camera_angles=gen_camera_angles, cur_nimg=cur_nimg, wandb_run=wandb_run)
        ...
        if phase in ["Dmain", "Dall"]:
            with torch.autograd.profiler.record_function("Dgen_forward"):
                with torch.no_grad():
                    gen_img, _gen_ws, patch_params = self.run_G(gen_z, gen_camera_angles, camera_angles_cond=gen_camera_angles_cond, update_emas=True, points=points, folding=folding)
                gen_logits = self.run_D(gen_img, key="gen", blur_sigma=blur_sigma, update_emas=True, patch_params=patch_params, camera_angles=gen_camera_angles, cur_nimg=cur_nimg, wandb_run=wandb_run)
        ...
        with torch.autograd.set_detect_anomaly(True):
            if phase in ["Dmain", "Dreg", "Dall"]:
                ...
                with torch.autograd.profiler.record_function(name + "_forward"):
                    (real_img, patch_params) = self.extract_patches(real_img) if self.patch_cfg["enabled"] else (real_img, None)
                    real_img_tmp = real_img.detach().requires_grad_(phase in ["Dreg", "Dall"])
                    real_logits = self.run_D(real_img_tmp, key="real", blur_sigma=blur_sigma, patch_params=patch_params, camera_angles=real_camera_angles, cur_nimg=cur_nimg, wandb_run=wandb_run)
