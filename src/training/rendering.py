...
def extract_density_from_sdf_grid(coords: torch.Tensor, sdf_grid: torch.Tensor, beta: torch.Tensor=None) -> torch.Tensor:
    ...
    # sdfs = F.grid_sample(sdf_grid, coords_normed.view(batch_size, 1, 1, num_points, 3), padding_mode="border", align_corners=False).view(batch_size, num_points, 1)
    sdfs = F.grid_sample(sdf_grid, coords_normed.view(batch_size, 1, 1, num_points, 3), padding_mode="border", align_corners=False).view(batch_size, num_points, 1)
...
@misc.profiled_function
def fancy_integration(rgb_sigma, z_vals, noise_std=0.5, bg_color=None, last_back=False, clamp_mode=None, fill_mode=None, sp_beta: float=1.0, use_inf_depth: bool=True):
    """
    Performs NeRF volumetric rendering over features or colors.
    Assumes that the last dimension is density.
    ...
    is_white = bg_color.expand_as(rgb_final)
    white_bg_rgb = is_white * rgb_final + is_white - is_white * weights_agg
    black_bg_rgb = (1-is_white) * rgb_final
    rgb_final = white_bg_rgb + black_bg_rgb
