...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode="random", fused_modconv=True, gain=1):
        ...
        x = modulated_conv2d(
            x=x, weight=self.weight, styles=styles, noise=noise, up=self.up, padding=self.padding, resample_filter=self.resample_filter, flip_weight=flip_weight, fused_modconv=fused_modconv
        )
    ...
    def extra_repr(self):
        return " ".join(
            [f"in_channels={self.in_channels:d}, out_channels={self.out_channels:d}, w_dim={self.w_dim:d},", f"resolution={self.resolution:d}, up={self.up}, activation={self.activation:s}"]
        )
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if in_channels != 0:
            self.conv0 = SynthesisLayer(
                in_channels, out_channels, w_dim=w_dim, resolution=resolution, up=2, resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=self.channels_last, **layer_kwargs
            )
            self.num_conv += 1
