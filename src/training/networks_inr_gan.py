...
class SynthesisLayer(torch.nn.Module):
    ...
    def extra_repr(self):
        return " ".join([f"in_channels={self.in_channels:d}, out_channels={self.out_channels:d}, w_dim={self.w_dim:d},", f"activation={self.activation:s}"])
    ...
    def __init__(self, cfg: DictConfig, w_dim: int, **block_kwargs):
        ...
        for block_idx in range(self.cfg.num_blocks):
            ...
            block = SynthesisBlock(
                in_channels, out_channels, w_dim=w_dim, img_channels=self.cfg.output_channels, is_last=is_last, use_fp16=use_fp16, cfg=cfg, coord_dim=self.cfg.coord_dim, **block_kwargs
            )
