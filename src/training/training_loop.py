...
def training_loop(
    ...
    if rank == 0:
        if not resume_whole_state:
            ...
            vis = dnnlib.EasyDict()
            vis.grid_size, images, vis.camera_angles, masks, vis.points, vis.folding = setup_snapshot_image_grid(training_set=training_set, cfg=cfg)
            ...
            try:
                wandb_run.log({"eval/real": wandb.Image(os.path.join(run_dir, "reals.jpg"))})
                wandb_run.log({"eval/reals_mask": wandb.Image(os.path.join(run_dir, "reals_mask.jpg"))})
            except:
                pass

            vis.grid_z = (torch.randn([images.shape[0], G.z_dim], device=device)).split(cfg.training.test_batch_gpu)
            vis.points = torch.from_numpy(vis.points).to(device).split(cfg.training.test_batch_gpu)
            vis.folding = torch.from_numpy(vis.folding).to(device).split(cfg.training.test_batch_gpu)
            vis.grid_camera_angles = torch.from_numpy(vis.camera_angles).to(device).split(cfg.training.test_batch_gpu)
        ...
        with torch.no_grad():
            images = torch.cat([G_ema(z=z, camera_angles=a, p=p, f=f, noise_mode="const").cpu() for z, a, p, f in zip(vis.grid_z, vis.grid_camera_angles, vis.points, vis.folding)]).numpy()
            videos, _ = generate_videos(
                G_ema,
                torch.stack(vis.grid_z).view(-1, G.z_dim),
                torch.stack(vis.points).view(images.shape[0], -1, 6),
                torch.stack(vis.folding).view(images.shape[0], -1, 9),
    ...
    while True:
        ...
        real_img, real_mask, phase_real_camera_angles, surface_points, folding_data = batch["image"], batch["mask"], batch["camera_angles"], batch["pointcloud"], batch["folding"]
        ...
        phase_folding_data = folding_data.split(batch_gpu)
        ...
        for phase, phase_gen_z, phase_gen_camera_angles, phase_gen_camera_angles_cond in all_data:
            ...
            phase_data = zip(phase_real_img, phase_real_camera_angles, phase_gen_z, phase_gen_camera_angles, phase_gen_camera_angles_cond, phase_surface_points, phase_folding_data)
            for real_img, real_camera_angles, gen_z, gen_camera_angles, gen_camera_angles_cond, points, folding in phase_data:
