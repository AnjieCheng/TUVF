...
class ImageFolderDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.fixed_view_mode:
            assert self.view_id is not None
            sampled_view = get_view_from_list(self.view_id)
        else:
        ...
        for c_i, c_v in zip(image_selections, sampled_view):
            angles = np.array([c_v["azimuth"] + math.pi, c_v["elevation"], 0, 1.75, c_i["fov"]]).astype(np.float)
            view_angles.append(angles)
        view_angles = np.stack(view_angles, axis=0)

        if self.folding_data is None:
            self.__load_folding_packed__()

        shapenet_id = self.dpsr_items[idx]
        ...
        assert shapenet_id in self.folding_data_keys
        obj_idx = self.folding_data_keys[shapenet_id]
        texturify_obj_name = self.folding_data["texturify_obj_names"][obj_idx]
        if "dense_folding_points_all" in self.folding_data:
            folding_points = self.folding_data["dense_folding_points_all"][obj_idx]
            folding_normals = self.folding_data["dense_folding_normals_all"][obj_idx]
            batch_p_2d = self.folding_data["dense_batch_p_2d_all"][obj_idx]
        else:
        ...
        return {
            "camera_angles": view_angles.astype(np.float32),
            ...
            "texturify_obj_name": texturify_obj_name,
        }
    ...
    def __load_folding_packed__(self):
        ...
        self.folding_data_keys = {k: v for v, k in enumerate(self.folding_data["shapenet_obj_names"])}
    ...
    def get_camera_angles(self, idx):
        ...
        angles = np.array([c_i["azimuth"] + math.pi, c_i["elevation"], 0, 1.575, c_i["fov"]]).astype(np.float)  # (5,)
        return angles
...
def get_view_from_list(view_id, num_views=4):
    num_views = 4
    azimuth = [np.linspace(start=math.pi - math.pi, stop=math.pi + math.pi, num=num_views).tolist()[view_id]]
    elevation = [np.linspace(start=0.785, stop=0.785, num=num_views).tolist()[view_id]]
