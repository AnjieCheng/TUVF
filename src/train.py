...
def subprocess_fn(rank, c, temp_dir):
    ...
    if rank == 0:
        try:
            ...
            #     entity="anjjei", project="uvgan", name=c.cfg.experiment_name_with_hash
            # )
...
def launch_training(c, outdir, dry_run):
    ...
    if c.cfg.env.get("symlink_output", None):
        print(
            f"Output directory:    {c.run_dir} (symlinked to {c.cfg.env.symlink_output})"
        )
    else:
    ...
    if c.cfg.env.get("symlink_output", None):
        ...
        if not c.resume_whole_state:
            ...
            print(
                f"Did not symlink `{c.cfg.env.symlink_output}` since resuming training."
            )
    else:
...
def init_dataset(cfg: DictConfig):
    try:
        ...
        elif "airplanes" in cfg.dataset.name:
            class_name = "src.training.dataset_airplanes.ImageFolderDataset"
        elif "single_horses" in cfg.dataset.name:
            class_name = "src.training.dataset_single_horses.ImageFolderDataset"
        elif "single_cats" in cfg.dataset.name:
            class_name = "src.training.dataset_single_cats.ImageFolderDataset"
        elif "single_dogs" in cfg.dataset.name:
            class_name = "src.training.dataset_single_dogs.ImageFolderDataset"
        elif "single_cows" in cfg.dataset.name:
            class_name = "src.training.dataset_single_cows.ImageFolderDataset"
        else:
        ...
        if cfg.dataset.sampling.dist == "custom":
            print("Validating camera poses in the dataset...")
            camera_angles = torch.from_numpy(
                np.array([dataset.get_camera_angles(i) for i in range(len(dataset))])
            )
            ...
            print("Camera poses validation done!")
        else:
            mean_camera_pose = torch.tensor(
                [
                    cfg.dataset.sampling.horizontal_mean,
                    cfg.dataset.sampling.vertical_mean,
                    0.0,
                ]
            )  # [3]
...
def main(cfg: DictConfig):
    ...
    c.G_kwargs = dnnlib.EasyDict(
        class_name=None,
        cfg=cfg.model.generator,
        z_dim=512,
        w_dim=cfg.model.generator.w_dim,
        mapping_kwargs=dnnlib.EasyDict(),
    )
    c.D_kwargs = dnnlib.EasyDict(
        class_name="training.networks_discriminator.Discriminator",
        cfg=cfg.model.discriminator,
    ...
    c.G_opt_kwargs = dnnlib.EasyDict(
        ...
        betas=list(cfg.model.generator.optim.betas),
        eps=1e-8,
    )
    c.D_opt_kwargs = dnnlib.EasyDict(
        class_name="torch.optim.Adam",
        betas=list(cfg.model.discriminator.optim.betas),
        eps=1e-8,
    )
    ...
    c.D_kwargs.channel_base = int(
        cfg.model.discriminator.cbase * cfg.model.discriminator.fmaps
    )
    ...
    c.G_kwargs.mapping_kwargs.camera_cond_drop_p = (
        cfg.model.generator.camera_cond_drop_p
    )
    ...
    c.D_kwargs.epilogue_kwargs.mbstd_group_size = (
        cfg.model.discriminator.mbstd_group_size
    )
    c.loss_kwargs.r1_gamma = (
        (0.0002 * (cfg.dataset.resolution**2) / opts.batch_size)
        if opts.gamma == "auto"
        else opts.gamma
    )
    c.G_opt_kwargs.lr = cfg.model.generator.optim.get(
        "lr", 0.002 if cfg.model.name == "stylegan2" else 0.0025
    )
    ...
    c.G_reg_interval = (
        4 if cfg.model.loss_kwargs.pl_weight > 0 else 0
    )  # Enable lazy regularization for G.
    ...
    if cfg.model.name == "stylegan2":
        ...
        elif cfg.model.name == "canograf_geo":
            print("using canograf_geo")
            c.G_kwargs.class_name = "training.networks_canograf_geo.Generator"
    ...
    if opts.augment.mode != "noaug":
        c.augment_kwargs = dnnlib.EasyDict(
            class_name="training.augment.AugmentPipe", **opts.augment.probs
        )
    ...
    if cfg.training.patch.enabled:
        if cfg.training.patch.distribution in ("uniform", "discrete_uniform", "beta"):
            assert (
                cfg.training.patch.min_scale_trg * cfg.dataset.resolution
                >= cfg.training.patch.resolution
            ), (
    ...
    if opts.resume == "latest":
        ...
        ckpts = (
            sorted([f for f in os.listdir(run_dir) if ckpt_regex.match(f)])
        ...
        if len(ckpts) > 0:
            ...
            warnings.warn(
                "Was requested to continue training, but cannot found any checkpoints. Please remove `training.resume=latest` argument."
            )
    elif opts.resume is not None:
        c.resume_pkl = opts.resume
        c.ada_kimg = 100  # Make ADA react faster at the beginning.
        c.ema_rampup = None  # Disable EMA rampup.
        c.cfg.training.loss_kwargs.blur_init_sigma = 0  # Disable blur rampup.
